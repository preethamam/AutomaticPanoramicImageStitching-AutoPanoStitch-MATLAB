// Compile (MSVC): mex -O COMPFLAGS="/O2 /openmp $COMPFLAGS" nearest2_hamming_exhaustive_omp_mex.cpp
// Compile (GCC/Clang/MinGW): mex -O CXXFLAGS="\$CXXFLAGS -O3 -march=native -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp" nearest2_hamming_exhaustive_omp_mex.cpp
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]){
    ...
    const mwSize N1mw = sa[0];
    const mwSize nb   = sa[1];
    const mwSize N2mw = sb[0];
    if (sb[1] != nb) mexErrMsgIdAndTxt("hamm2nn:cols","Byte width mismatch.");

    const int N1 = (int)N1mw;
    const int N2 = (int)N2mw;

    const uint8_t* Ap = (const uint8_t*)mxGetData(A);  // N1 x nb (col-major)
    const uint8_t* Bp = (const uint8_t*)mxGetData(B);  // N2 x nb (col-major)

    plhs[0] = mxCreateNumericMatrix(N1mw,1,mxUINT32_CLASS,mxREAL);
    plhs[1] = mxCreateNumericMatrix(N1mw,1,mxSINGLE_CLASS,mxREAL);
    plhs[2] = mxCreateNumericMatrix(N1mw,1,mxSINGLE_CLASS,mxREAL);
    ...
    if (N2==0){
        for (int i=0;i<N1;++i){ idx2[i]=0; d1[i]=mxGetNaN(); d2[i]=mxGetNaN(); }
        return;
    }
    ...
    for (int i = 0; i < N1; ++i) {
        ...
        for (int j = 0; j < N2; ++j) {
            ...
            for (mwSize b = 0; b < nb; ++b) {
                const uint8_t ai = Ap[(mwSize)i + b*N1mw];
                const uint8_t bj = Bp[(mwSize)j + b*N2mw];
