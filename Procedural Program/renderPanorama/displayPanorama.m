function panoStore = displayPanorama(input, finalPanoramaTforms, ...
    ...
    %                         • canvasColor, gainCompensation, sigmaN, sigmag, bands,
    ...
    %
    % Notes
    %   - The focal length for planar projection is taken as fPan = cameras(refIdx).K(1,1).
    ...
    for ii = 1:length(finalPanoramaTforms)
        ...
        % Example call after BA
        opts = struct('fPan', cameras(refIdx).K(1, 1), 'resScale', 1.0, ...
            'anglePower', 2, 'cropBorder', true, ...
            'canvasColor', input.canvasColor, ...
            'gainCompensation', input.gainCompensation, ...
            'sigmaN', input.sigmaN, ...
            'sigmag', input.sigmag, ...
            'pyrLevels', input.bands, ...
            'blending', input.blending, ...
            'pyrSigma', input.MBBsigma, ...
            'showPanoramaImgsNums', input.showPanoramaImgsNums, ...
            'showCropBoundingBox', input.showCropBoundingBox, ...
            'useMATLABImwarp', input.useMATLABImwarp);

        % Render all projections panorama
        for jj = 1:numel(input.panorama2DisplaynSave)
            projection = input.panorama2DisplaynSave(jj);
            [panorama, annoRGB] = renderPanorama(input, images, imageSizes, cameras, projection, refIdx, opts);
