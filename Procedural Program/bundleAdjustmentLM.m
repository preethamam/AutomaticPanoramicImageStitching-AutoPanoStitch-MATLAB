function [cameras, seed] = bundleAdjustmentLM(input, numMatches, matches, keypoints, ...
    ...
    if ~isfield(opts, 'MaxMatches'), opts.MaxMatches = Inf; end % no cap by default
    ...
    for step = 2:N
        ...
        cameras = runLevenbergMarquardt( ...
            ...
            matches, keypoints, imageSizes, opts_iter);
    end
    ...
    if numel(initializedList) > 1
        ...
        for pass = 1:nPasses
            ...
            cameras = runLevenbergMarquardt( ...
                cameras, initializedList, seed, ...
                matches, keypoints, imageSizes, opts_final);
        end
...
function pairs = build_pairs_from_cells(matches, keypoints)
    % BUILD_PAIRS_FROM_CELLS Create compact pair list from match and keypoint cells.
    %   pairs = build_pairs_from_cells(matches, keypoints) returns an array of
    %   structs with fields i, j, Ui, Uj where Ui/Uj are M×2 pixel coordinates
    %   for the matched features. Only upper-triangular (i<j) non-empty pairs
    %   are included.
    arguments
        matches (:, :) cell
        keypoints cell
...
function [r_stacked, Ji, Jj, E_sum, r2sum, rcnt] = jacobian_pair( ...
    ...
    arguments
        ...
        cam_i_base (1, 1) struct
        cam_j_base (1, 1) struct
        cam_i_lin (1, 1) struct
        cam_j_lin (1, 1) struct
        size_i (1, :) double {mustBeFinite}
        size_j (1, :) double {mustBeFinite}
        sigmaHuber (1, 1) double {mustBeFinite, mustBePositive}
        cols_i (1, :) double {mustBeInteger, mustBePositive}
        cols_j (1, :) double {mustBeInteger, mustBePositive}
        opts struct = struct
    end
...
function delta_c = cap_per_camera_step(delta, pmap, cameras, camList, seed, theta_cap, frac_df)
    % CAP_PER_CAMERA_STEP Limit per-camera parameter update to stabilize LM.
    %   Applies a rotation step cap (radians) and a fractional cap for f.
    ...
    arguments
        ...
        camList (1, :) double {mustBeInteger, mustBePositive}
        seed (1, 1) double {mustBeInteger, mustBePositive}
        theta_cap (1, 1) double {mustBePositive, mustBeFinite}
        frac_df (1, 1) double {mustBePositive, mustBeFinite}
    end
...
function [r, J_obs, J_src] = computeSingleResidual(u_obs, u_src, cam_obs, cam_src, ...
    ...
    % --- Arguments validation ---
    arguments
        u_obs double {mustBeFinite}
        u_src double {mustBeFinite}
        cam_obs (1, 1) struct
        cam_src (1, 1) struct
        imageSize_obs (1, :) double {mustBeFinite}
        imageSize_src (1, :) double {mustBeFinite}
    end
...
function J = computeJacobianWrtCamera(u_src_h, cam_obs, cam_src, p_h, type)
    ...
    % --- Arguments validation ---
    arguments
        u_src_h (3, 1) double {mustBeFinite}
        cam_obs (1, 1) struct
        cam_src (1, 1) struct
        p_h (3, 1) double {mustBeFinite}
...
function cameras = prepareCameraCache(cameras, imageSizes)
    ...
    %   struct has fields cx,cy and K consistent with its focal length.
    %
    % --- Arguments validation ---
    arguments
        cameras (1, :) struct
...
function Hji = getHomog_j_to_i(input, pair, imageSizes, initialTforms) %#ok<INUSD>
    ...
    arguments
        input (1, 1) struct
        pair (1, 1) struct
        imageSizes (:, :) double {mustBeFinite}
        initialTforms
    end
